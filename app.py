...
# ── Reward configuration (change here to adjust future point values) ───────
POINTS_FOR_RESOLVED_COMPLAINT = 50
...
def dashboard():
    ...
    user_pts = query("SELECT points FROM users WHERE id=?", (uid,), one=True)
    points   = user_pts['points'] if user_pts else 0
...
def admin_panel():
    ...
    # Reward analytics
    pts_row       = query("SELECT COALESCE(SUM(points),0) as t FROM users WHERE role='student'", one=True)
    total_pts     = pts_row['t'] if pts_row else 0
    top_students  = query("""SELECT name, email, points FROM users
...
def update_complaint(cid):
    ...
    # ── Reward points logic (anti-abuse: reward_given guard) ──────────────
    complaint = query("SELECT user_id, reward_given FROM complaints WHERE id=?",
                      (cid,), one=True)
    reward_msg = ''
    if complaint and status == 'Assigned' and complaint['reward_given'] == 0:
        # Award points exactly once when assigned by admin
        execute("UPDATE users SET points = points + ? WHERE id=?",
                (POINTS_FOR_RESOLVED_COMPLAINT, complaint['user_id']))
        execute("UPDATE complaints SET reward_given=1 WHERE id=?", (cid,))
        reward_msg = f' +{POINTS_FOR_RESOLVED_COMPLAINT} pts awarded to student.'

    execute("""UPDATE complaints SET status=?, priority=?, updated_at=datetime('now')
               WHERE id=?""", (status, priority, cid))
    flash('Complaint updated successfully.' + reward_msg, 'success')
    return redirect(url_for('admin_panel'))
...
def leaderboard():
    top_students = query("""
        SELECT id, name, email, points,
               (SELECT COUNT(*) FROM complaints
                WHERE user_id=users.id AND status='Resolved') as resolved_count
        FROM users
        WHERE role='student'
        ORDER BY points DESC, resolved_count DESC
        LIMIT 50
    """)
    # Current user's rank
    user_rank = None
    for i, s in enumerate(top_students, 1):
        if s['id'] == session['user_id']:
            user_rank = i
            break
    my_points = query("SELECT points FROM users WHERE id=?",
                      (session['user_id'],), one=True)
    my_pts = my_points['points'] if my_points else 0
